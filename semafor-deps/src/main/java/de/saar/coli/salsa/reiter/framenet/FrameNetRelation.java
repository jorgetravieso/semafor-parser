...
/**
 * This class represents a relation between frames.  
 * The name as well as the frames on both ends can be retrieved.
 *
 * <p>This class is not instantiated directly, but through the
 * method {@link FrameNet#getFrameNetRelation(String)} or {@link FrameNet#getFrameNetRelations()}.
 * An instance of this class does not represent a single relation between
 * a frame and another frame, but rather all relations of the given kind. </p>
 * 
 * <h3>Example</h3>
 * <code>
 ...
 *
 */
public abstract class FrameNetRelation implements Serializable, IHasName, IHasID {
	...
	/**
	 * Returns the other frame(s) in this relation. This may either be the
	 * super or sub frame, depending on the direction given
	 * 
	 * 
	 ...
	 */
	public Collection<Frame> getOther(Frame frame, FrameNetRelationDirection direction) {
	...
	/**
	 * Returns the name of the sub frames, i.e., "Child" in the case of inheritance
	 * 
	 * @return the subName
	 */
	...
	/**
	 * Returns the name of the super frames, i.e., "Parent" in the case of inheritance
	 * 
			...
			if (frel.getSubFrame().equals(frame) || 
				...
				for (FrameElementRelation ferel : frel.getFrameElementRelations()) {
					if (ferel.getSubFrameElement().equals(frameElement) ||
						ferel.getSuperFrameElement().equals(frameElement))
						return true;
				}
		}
		return false;
