...
public class AnnotationSet implements IHasID {
	 ...
	 * @throws FrameElementNotFoundException
	 */
	public AnnotationSet(Sentence sentence, Element element, Frame frame, LexicalUnit lexicalUnit) 
	...
	private void processFrameElements(Element element) throws FrameElementNotFoundException, JaxenException {
		List felabels = new Dom4jXPath("layers/layer[@name='FE']/labels/label").selectNodes(element);
					...
					try {
						if (label.attributeValue("start") != null && label.attributeValue("end") != null) {
							...
							if (frame != null) {
								...
								DefaultAttribute gfAttribute = (DefaultAttribute) new Dom4jXPath("../../../layer[@name='GF']/labels/label[@start='"+start+"' and @end='"+end+"']/@name").selectSingleNode(label);
								if (gfAttribute != null)
									rfe.setProperty("GF", gfAttribute.getStringValue());

								DefaultAttribute ptAttribute = (DefaultAttribute) new Dom4jXPath("../../../layer[@name='PT']/labels/label[@start='"+start+"' and @end='"+end+"']/@name").selectSingleNode(label);
								if (gfAttribute != null)
