...
/**
 * This class parses a very simple format for FrameNet annotated text, 
 * that can easily be generated by scripting languages. 
 * 
 * The annotation is stored line-by-line. Each line starts with one of the three
 * tags: text, frame or fe. An annotated sentence usually starts with a text-line:
 * 
 * <blockquote><code>text The parishioners of St. Michael and All Angels stop to chat at the church door , as members here always have .</code></blockquote>
 * 
 ...
 * 
 * The frame elements are associated with the preceeding frame, it is therefore possible to specify 
 * more than one frame annotation for one text. 
 * 
...
public class  FlatFormat extends CorpusReader {	
	 ...
	 */
	public FlatFormat (FrameNet frameNet, Logger logger) {
		super(frameNet, logger);
	}

	/**
	 * This method parses the given file and stores the found annotation sentence-wise in the object.
	 * 
	 */
	public void parse(File file) 
	throws FileNotFoundException, ParsingException, FrameNotFoundException, FrameElementNotFoundException {
		BufferedReader br;

		if (!file.exists() || !file.canRead()) {
			this.getLogger().severe("FlatFormat.process(): cannot read " + file.getName());
		}
		else {
			try {
				...
				do {
					...
					if (br.ready()) {
						...
						if (token[0].equals("frame")) {
							...
							try {
								IToken tok = currentSentence.getTokenForString(join(' ', tokens.subList(2, tokens.size())));
								if (tok != null)
									current = getFrameNet().getFrame(token[1]).realize(tok);
						...
						if (token[0].equals("fe") && current != null) {
							try {
								IToken tok = currentSentence.getTokenForString(join(' ', tokens.subList(2, tokens.size())));
								if (tok != null)
									current.addRealizedFrameElement(token[1], tok);
						...
						if (token[0].equals("text")) {
							if (currentSentence != null)  {
								...
								this.getSentenceIndex().put(currentSentence.getId(), currentSentence);
							}
	...
	private String join(char c, Collection<String> a) {
		String ret = "";
		boolean first = true;
		for (String s : a) {
			if (first) {
