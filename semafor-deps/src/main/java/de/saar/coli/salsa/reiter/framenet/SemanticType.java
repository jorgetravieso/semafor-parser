...
/**
 * This class represents the semantic types in FrameNet.
 * It is a very rudimental implementation. It collects 
 * all frame elements that use a specific semantic type, but
 * it does not allow browsing through the hierarchy of semantic types.
 * 
 ...
 *
 */
public abstract class SemanticType implements Serializable, IHasNameAndID, IHasDefinition, IHasTrees {
	...
	/**
	 * Adds a frame element to the list of frame elements that use this
	 * semantic type.
	 * @param frameElement The frame element to add.
	 */
	...
	private void init() {
		if (this.frameElements == null)
			this.frameElements = new HashSet<FrameElement>();
		if (this.superTypes == null)
			this.superTypes = new HashSet<SemanticType>();
		if (this.subTypes == null)
			this.subTypes = new HashSet<SemanticType>();
	...
	/**
	 * Adds a {@link FrameElement} to the list of frame elements which
	 * use this semantic type
	 * 
	 * @param frameElement A frame element that uses this ST.
	 */
	protected void registerFrameElement(FrameElement frameElement) {
		init();
		this.frameElements.add(frameElement);
	}
	
	/**
	 * Stores a semantic type as subtype of this ST.
	 * @param semanticType The new subtype
	 */
	protected void registerSubType(SemanticType semanticType) {
		init();
		this.subTypes.add(semanticType);
	}
	...
	/**
	 * Returns a tree-like representation of the inheritance hierarchy on top
	 * of this hierarchy
	 */
	...
	private String tree(String prefix, boolean up) {
		...
		if (up) others = this.getSuperTypes();
		else others = this.getSubTypes();
