...
/**
 * This class represents a single sentence in the FrameNet corpus. 
 * Multiple annotation sets belong to one sentence.
 * 
 * 
 * @author Nils Reiter
 * @since 0.2
...
public class Sentence extends de.saar.coli.salsa.reiter.framenet.Sentence {
	...
	/**
	 * This constructor takes a FrameNetCorpus object and the 
	 * XML element node representing a single sentence in the 
	 * data file.
	 * @param corpus The FrameNetCorps
	 * @param node The sentence node
	...
	throws FrameElementNotFoundException, FrameNotFoundException, ParsingException, JaxenException {
		super(node.attributeValue("ID"), 
				node.element("text").getText());
		this.corpus = corpus;
		
		// Search for all tokens, add them to the list
		List allTokens = new Dom4jXPath("annotationSets/annotationSet/layers/layer/labels/label[@start and @end]").selectNodes(node);
		for (Object tokenObj : allTokens) {
			Element token = (Element) tokenObj;
			Range range = 
				new Range(
						new Integer(token.attributeValue("start")),
						new Integer(token.attributeValue("end"))+1);
			this.addToken(range);
		}
		
		List asets = new Dom4jXPath("annotationSets/annotationSet[@frameName]").selectNodes(node);
		...
		for (Object aseto : asets) {
			...
			if (aset.attributeValue("frameName") != "") {
				try {
					...
					this.getCorpus().getLogger().severe(e.getMessage());
					if (corpus.isAbortOnError())
						throw new ParsingException(e.getMessage());
				} catch (FrameNotFoundException e) {
					this.getCorpus().getLogger().warning(e.getMessage());
					if (corpus.isAbortOnError())
	 ...
	 * 
	 * @param annotationCorpus The corpus
	 * @param element The XML element of the sentence
	 * @throws JaxenException
	 */
	@SuppressWarnings("unchecked")
	public Sentence(AnnotationCorpus annotationCorpus, Element element) throws JaxenException {
		super(element.attributeValue("ID"), element.element("text").getText());
		
		List allTokens = new Dom4jXPath("layers/layer/labels/label[@start and @end]").selectNodes(element.getParent());
		for (Object tokenObj : allTokens) {
			Element token = (Element) tokenObj;
			Range range = 
				new Range(
