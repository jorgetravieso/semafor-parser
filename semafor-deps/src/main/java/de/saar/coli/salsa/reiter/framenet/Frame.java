...
implements Serializable, IHasCDate, IHasDefinition, IHasNameAndID, IHasTrees {
	...
	public boolean linkFrameNet(FrameNet fn) {
		if (! ( this.getName() != null && 
			 this.getCDate() != null &&
			 this.getId() != null &&
			 this.getDefinition() != null) ) return false;
		this.framenet = fn;
	...
	public Collection<Frame> relatedFrames(String relation, FrameNetRelationDirection frameNetRelationDirection) {
		...
		if (! frameRelations.get(fnrel).containsKey(frameNetRelationDirection))
			frameRelations.get(fnrel).put(frameNetRelationDirection, fnrel.getOther(this, frameNetRelationDirection));
	...
	public Integer distance(Frame other)  {
		if (this.equals(other)) return new Integer(0);	
		if (distances.containsKey(other)) return distances.get(other);
	...
	private String tree(String prefix, boolean up) {
		...
		if (up) others = this.inheritsFrom();
		else others = this.isInheritedBy();
		for (Frame f : others) {
	...
	public boolean isIsolated() {
		return (this.inheritsFrom().isEmpty() &&
				this.isInheritedBy().isEmpty());
