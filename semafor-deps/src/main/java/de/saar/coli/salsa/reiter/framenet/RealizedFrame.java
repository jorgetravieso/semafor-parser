...
/**
 * This class represents a frame and a target (a "word" in a sentence). 
 * The target may be some id, depending on the properties of the 
 * parsing procedure. 
 * 
 * @author Nils Reiter
 *
...
public class RealizedFrame implements IHasTarget, IHasID {
	 ...
	 * The constructor 
	 * 
	 * @param frame The frame to be realized
	...
	/**
	 * Adds a realized frame element to the realized frame.
	 * The frame element has to belong to this frame
	 * @param realizedFrameElement The realized frame element
	 */
	public void addRealizedFrameElement(RealizedFrameElement realizedFrameElement) {
		//if (! this.getFrame().frameElements().contains(realizedFrameElement.frameElement))
		//	throw new FrameElementNotFoundException(this.getFrame(), realizedFrameElement.frameElement);
		frameElements.put(realizedFrameElement.frameElement.name, realizedFrameElement);
	}
	
	/**
	 * Adds a new realized frame element. Sets xmlid to 0.
	 * @param fename The name of the frame element
	 * @param target The target of the frame element
	 * @throws FrameElementNotFoundException If the frame element does not belong to the frame
	 * @return The realized frame element
	 */
	public RealizedFrameElement addRealizedFrameElement(String fename, IToken target) 
	throws FrameElementNotFoundException  {
		RealizedFrameElement rfe = new RealizedFrameElement(this, frame.getFrameElement(fename), target, "");
	 ...
	 * @param fename The name of the frame element
	 * @param start The first character of the frame element target
	 * @param end The first character after the frame element target
	 * @throws FrameElementNotFoundException This exception is thrown when the frame element
	 * does not belong to the given frame
	 */
	/*public void addRealizedFrameElement(String fename, int start, int end)
	throws FrameElementNotFoundException{
		frameElements.put(fename, new RealizedFrameElement(this, frame.getFrameElement(fename), start, end));
	}*/
	...
	/**
	 * Returns a sorted set of realized frame elements that have been set for this
	 * frame
	 * The realized frame elements are sorted according to the starting character 
	 * position of the target. If none has been specified, the ordering is undefined.
	 * @return A sorted set of RealizedFrameElements
	 */
	public SortedSet<RealizedFrameElement> frameElements() {
		SortedSet<RealizedFrameElement> ret = new TreeSet<RealizedFrameElement>(new Comparator<RealizedFrameElement>() {
			public int compare(RealizedFrameElement r1, RealizedFrameElement r2) {
				...
				if (r1.getStart() == r2.getStart()) {
					...
					return new Integer(r2.getEnd()).compareTo(r2.getEnd());
				}
				return new Integer(r1.getStart()).compareTo(r2.getStart());
			}
		});
	...
	/**
	 * Generalizes all content of this frame. Returns all possible generalizations
	 ...
	 * @param frame The frame to which one wants to generalize
	...
	/**
	 * This method returns true if there is a frame to which this 
	 * frame can be generalized
	 * @return true or false
	 */
	public boolean generalizable() {
		for (Frame frame : this.frame.inheritsFrom()) {
			if (generalizable(frame)) return true;
		}
		return false;
	}
	/**
	 ...
	 * @param frame The frame to which one wants to generalize
	 * @return The generalized realized frame or this, if no generalization is possible
	 */
	public RealizedFrame generalize(Frame frame)  {
		// TODO (fixed): Generalization of realized frames does not work completly ... 
		...
		if (getStart() != -1)
			gFrame.setStart(getStart());
		if (getEnd() != -1)
			gFrame.setEnd(getEnd());
		if (getFrame().inheritsFrom().isEmpty() ||
			! getFrame().inheritsFrom().contains(frame)) return this;
			...
			for(RealizedFrameElement rfe : frameElements()) {
				...
				if (gFrameElement == null)
					return this;
				RealizedFrameElement gfe = gFrameElement.realize(gFrame, rfe.getTarget(), rfe.getId());
				gfe.setStart(rfe.getStart());
				gfe.setEnd(rfe.getEnd());
			}
			//generalizations.put(frame, gFrame);
			return gFrame;
	...
	/**
	 * Returns the realized frame elements in this realized frame.
	 * Note, that this method only returns the frame elements that
	 * are actually used.
	 * 
