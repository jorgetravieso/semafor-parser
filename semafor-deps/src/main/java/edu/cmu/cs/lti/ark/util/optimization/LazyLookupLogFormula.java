 ...
 *
 * 2010-11-08 (nschneid): Added support for multithreading wherein each training example gets its own thread. Only {@link
 * LazyLookupLogFormula#add_arg(LogFormula)} nodes may be shared by multiple training examples. {@link #backprop(LogModel, LDouble)}
 * and {@link #lookup_backprop(LogModel, LDouble)} are thus synchronized.
 *
...
public class LazyLookupLogFormula extends LogFormula {
    ...
    /**
     * DON'T reset m_value or m_gradient since these may be references to elements in the Model's table. Instead, we'll just replace
     * the references in lookup_evaluate and lookup_backprop below.
     */
