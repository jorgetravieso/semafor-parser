 ...
 * LazyLookupLogFormula#add_arg(LogFormula)} nodes may be shared by multiple training examples.
 *
...
public class RootLogFormula extends LogFormula {
    ...
    protected synchronized void _updateValue(LDouble v) {
        switch (m_operation) {
            case TIMES:
                LogMath.logtimes(m_value, v, m_value);
                break;
            case PLUS:
                LogMath.logplus(m_value, v, m_value);
                break;
            default:
                System.out.println(String.format("Can't use operation %d with a RootFormula.", m_operation));
                break;
        }
    }
