...
/**
 * @author dipanjan This class is for finding the best frame for a span of tokens, given the annotation for a sentence. It is just a
 *         single node implementation. Moreover, it looks only at a small set of frames for seen words
 */
public class FastFrameIdentifier extends LRIdentificationModelSingleNode {
    ...
    private int K;
    private FeatureExtractor featex;
    ...
    public String getBestFrame(String frameLine, String[][] data) {
        ...
        if (set == null) {
            set = mFrameMap.keySet();
            System.out.printf("Semafor did not find frame line %s\n", frameLine);
        }
        for (String frame : set) {
            double val = getNumeratorValue(frame, intTokNums, data);
            if (val > maxVal) {
