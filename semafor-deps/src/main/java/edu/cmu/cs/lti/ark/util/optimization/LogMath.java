...
public class LogMath {
    ...
    /**
     * If lx.sign == true, treat exp(lx.value) as positive; otherwise, treat it as negative. So, If lx.sign == true and ly.sign ==
     * true, returns ln(exp(lx.value) + exp(ly.value)). If lx.sign == false and ly.sign == true, returns ln(-exp(lx.value) +
     * exp(ly.value)). If lx.sign == true and ly.sign == false, returns ln(exp(lx.value) - exp(ly.value)). If lx.sign == false and
     * ly.sign == false, returns ln(-exp(lx.value) - exp(ly.value)).
     */
    public static LDouble logplus(LDouble lx, LDouble ly) {
        ...
        if (lx.sign && !ly.sign) {
            return logminus(lx, new LDouble(ly.value, true));
        }
        if (!lx.sign && ly.sign) {
            return logminus(ly, new LDouble(lx.value, true));
        }
    ...
    public static LDouble logminus(LDouble lx, LDouble ly) {
        ...
        if (lx.sign && !ly.sign) {
            return logplus(lx, new LDouble(ly.value, true));
        }
        if (!lx.sign && ly.sign) {
            return logplus(lx, new LDouble(ly.value, false));
        }
        ...
        if (lx.value == negInf || ly.value - lx.value > 700.0) {
            return new LDouble(ly.value, !ly.sign);
        }
