...
public class SingleAssignmentHashMap<K, V> extends THashMap<K, V> implements java.io.Serializable {
    /**
     * What to do if {@link SingleAssignmentHashMap#put(Object, Object)} is invoked for a key that already has a non-{@code null}
     * value. <ul> <li>{@link #ERROR} indicates that an error should be printed</li> <li>{@link #IGNORE} indicates that the new
     * value should be ignored without error</li> <li>{@link #REPLACE} indicates that the new value should replace the old one
     * without error</li> <li>{@link #DIFFERENT} indicates that an error should be printed if the new value is different from the
     * old one; if they are the same (comparison via {@code equals()}) the new value will be ignored silently</li> </ul>
     *
     * {@link #ERROR} is the default for a {@link SingleAssignmentHashMap}. Such a policy can be specified at the instance level via
     * a constructor, or as an argument to {@link SingleAssignmentHashMap#put(Object, Object, ReassignmentPolicy)} (this will
     * override any instance-level policy).
     */
    public enum ReassignmentPolicy {
        ERROR, DIFFERENT, IGNORE, REPLACE
    }
    ...
    /**
     * Invokes {@link #put(Object, Object, ReassignmentPolicy)} with this instance's reassignment policy ({@link
     * ReassignmentPolicy#ERROR} by default).
     */
    @Override
    ...
    /**
     * Puts the value 'newValue' in the hash map with key 'key', subject to the reassignment policy (which matters if the current
     * value stored for 'key' is non-{@code null}). {@link #reset(Object)} can be called to restore the value to {@code null}.
     * Equivalent to {@link #putIfAbsent(Object, Object)}.
     *
